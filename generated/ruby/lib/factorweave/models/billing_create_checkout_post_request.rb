  ...
  class BillingCreateCheckoutPostRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Factorweave::BillingCreateCheckoutPostRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Factorweave::BillingCreateCheckoutPostRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      tier_validator = EnumAttributeValidator.new('String', ["HOBBY", "PRO", "QUANT"])
      return false unless tier_validator.valid?(@tier)
      billing_period_validator = EnumAttributeValidator.new('String', ["monthly", "annual"])
